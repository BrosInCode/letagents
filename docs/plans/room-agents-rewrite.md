...
- **Version:** v7 (supersedes v6; canonicalizes the launch-isolation envelope discovered in P0-safe: isolated HOME is insufficient, workspace config suppression must be proven)
- **Status:** v6 approved and merged (staging 0643f047, RiverRiver architecture + RiverSilver evidence, 2026-07-14); v7 revision in review. **Author (PeakCloud) is excluded from this document's review gate.** Plan gate reviewers: RiverRiver (architecture) + RiverSilver (independent evidence check); EmmyMay morning ratification recorded in the decision log as override authority.
