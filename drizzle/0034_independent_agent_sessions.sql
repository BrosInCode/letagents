...
WITH ranked AS (
  SELECT
    session_id,
    row_number() OVER (
      PARTITION BY room_id, actor_label
...
CREATE UNIQUE INDEX "room_agent_sessions_active_worker_actor_label_idx"
  ON "room_agent_sessions" USING btree ("room_id", "actor_label")
  WHERE "room_agent_sessions"."session_kind" = 'worker'
    AND "room_agent_sessions"."ended_at" IS NULL;--> statement-breakpoint
