  ...
  <section class="settings-panel settings-renting-panel" data-testid="settings-renting-pane">
    ...
    <template v-else-if="settings">
      <article class="settings-control-row">
        <div><strong>Available on this Mac</strong><p class="settings-control-description">Accept requests while this Mac and at least one agent are ready.</p></div>
      ...
      <article class="settings-renting-status"><div><span class="settings-renting-live-dot" :data-state="settings.daemonState"></span><strong>{{ { online: "Connected", offline: "Offline", starting: "Connecting…", error: "Connection problem" }[settings.daemonState] }}</strong></div><span>{{ settings.blockers.length ? `${settings.blockers.length} setup item${settings.blockers.length === 1 ? '' : 's'}` : 'Ready for requests' }}</span></article>
      ...
      <section class="settings-renting-section"><header><div><p class="settings-nav-heading">Available agent apps</p><span>Sign in to an agent app that supports isolated rental work before listing it.</span></div></header><div class="settings-control-list"><p v-if="settings.runtimes.length === 0" class="settings-renting-loading">No supported agent app was found on this Mac.</p><article v-for="runtime in settings.runtimes" :key="runtime.providerId" class="settings-control-row"><div><strong>{{ runtime.label }}</strong><p class="settings-control-description">{{ runtime.authenticated ? runtime.detail : 'Sign in to this agent app on your Mac before listing it.' }}</p></div><button class="settings-renting-switch" type="button" role="switch" :aria-checked="runtime.enabled" :data-on="runtime.enabled" :disabled="saving || (!runtime.enabled && (!runtime.authenticated || runtime.status !== 'ready' || !runtime.permissionProfileIds.length))" @click="toggleRuntime(runtime.providerId, !runtime.enabled)"><span></span></button></article></div></section>
      ...
      <article class="settings-renting-capabilities"><strong>Available now</strong><p>Rented agents work in a separate temporary folder with restricted file and command access. Your LetAgents and GitHub sign-in tokens are not shared with them. Access to repositories is currently unavailable.</p></article>
    </template>
