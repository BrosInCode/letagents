<template>
  <section class="settings-panel settings-renting-panel" data-testid="settings-renting-pane">
    ...
    <template v-else-if="settings">
      ...
      <section class="settings-renting-section">
        <header><div><p class="settings-nav-heading">Local runtimes</p><span>Only authenticated runtimes with a verified rental sandbox can be listed.</span></div></header>
        <div class="settings-control-list">
          <p v-if="settings.runtimes.length === 0" class="settings-renting-loading">No supported local runtime was found on this Mac.</p>
          <article v-for="runtime in settings.runtimes" :key="runtime.providerId" class="settings-control-row">
            <div>
              <strong>{{ runtime.label }}</strong>
              <p class="settings-control-description">{{ runtime.detail }}</p>
            </div>
            <div class="settings-renting-runtime-action">
              <button
                v-if="runtime.enabled || runtime.rentalSandboxStatus === 'verified'"
                class="settings-renting-switch"
                type="button"
                role="switch"
                :aria-label="`${runtime.enabled ? 'Disable' : 'Enable'} ${runtime.label} for renting`"
                :aria-checked="runtime.enabled"
                :data-on="runtime.enabled"
                :disabled="saving || (!runtime.enabled && runtime.rentalSandboxStatus !== 'verified')"
                @click="toggleRuntime(runtime.providerId, !runtime.enabled)"
              ><span></span></button>
              <span v-else-if="runtime.rentalSandboxStatus === 'unsupported'" class="state-pill" data-state="offline">Not available yet</span>
              <button
                v-else
                class="ghost-button"
                type="button"
                :disabled="saving || Boolean(verifyingRuntime)"
                @click="verifyRuntime(runtime.providerId)"
              >{{ verifyingRuntime === runtime.providerId ? 'Verifying…' : 'Verify sandbox' }}</button>
            </div>
          </article>
        </div>
      </section>
...
const verifyingRuntime = ref<DesktopRentalRuntimeId | null>(null);
...
async function verifyRuntime(providerId: DesktopRentalRuntimeId): Promise<void> { if (!desktopIpc.rental?.verifyProviderRuntime) return; verifyingRuntime.value = providerId; error.value = null; try { settings.value = await desktopIpc.rental.verifyProviderRuntime(providerId); } catch (cause) { error.value = cause instanceof Error ? cause.message : 'Could not verify the rental sandbox.'; } finally { verifyingRuntime.value = null; } }
