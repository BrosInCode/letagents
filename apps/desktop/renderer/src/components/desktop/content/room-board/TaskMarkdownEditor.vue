    ...
    <div class="task-markdown-surface">
      <div class="task-markdown-toolbar" role="group" aria-label="Markdown formatting">
        <button v-for="tool in markdownTools" :key="tool.id" type="button" class="task-markdown-tool" :data-tool="tool.id"
          ...
          <component :is="icons[tool.id]" :size="16" aria-hidden="true" />
        </button>
...
import { Bold, Italic, Strikethrough, Heading2, List, ListOrdered, ListTodo, Quote, SquareCode, Link } from '@lucide/vue'
import { renderDesktopMarkdown } from '../formatting/markdown'
const icons = { bold: Bold, italic: Italic, strike: Strikethrough, heading: Heading2, bullet: List, number: ListOrdered, check: ListTodo, quote: Quote, code: SquareCode, link: Link }
...
const rendered = computed(() => renderDesktopMarkdown(props.modelValue, { block: true, mentions: false, preservePaths: true }))
