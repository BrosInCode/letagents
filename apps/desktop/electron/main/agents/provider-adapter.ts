...
export interface ProviderSpawnRequest {
  ...
  /** Requested lifecycle authority, frozen when the returned native birth is checkpointed. */
  lifecycleAuthorityMode?: "legacy" | "typed_shadow" | "typed";
  /** Dormant, attested borrowed-worker profile. Launch returns idle; it never starts polling. */
  pollingContract?: "custodial_polling_v1";
  /** Durable manifest identity to register in the room; never generated by the adapter. */
  agentDisplayName?: string;
  /** The daemon-owned per-attempt workspace; never the user's dev checkout. */
  cwd: string;
  /** Explicit daemon-owned workspace boundary; managed adapters never infer it from agent ids or ancestor Git state. */
  workspaceKind?: "git_worktree" | "room_scratch";
  /**
