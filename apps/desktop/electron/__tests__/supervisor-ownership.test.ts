...
test("lane conflict distinguishes a blocked startup from a healthy existing owner", () => {
  assert.equal(
    ...
    "Codex supervised agent already reserved this room's supervised codex lane, but startup is recovering (coordination_blocked). Cause: provider restart_fresh failed: invalid placeholder continuation. Use the supervised runtime recovery card below and choose Stop before creating a replacement.",
  );
  assert.equal(
    describeSupervisorLaneConflict({
      displayName: "Codex supervised agent",
