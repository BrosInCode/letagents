...
test("desktop replaces the prior 2.0.45 implementation and accepts only the new exact implementation", async () => {
  const env = await fixture();
  const previous = process.env.LETAGENTS_ALLOW_NON_DARWIN_DAEMON;
  process.env.LETAGENTS_ALLOW_NON_DARWIN_DAEMON = "1";
  let oldServer: Server | null = null;
  ...
  oldServer = old.server;
  try {
    assert.notEqual("2.0.45", SUPERVISOR_DAEMON_IMPLEMENTATION_VERSION);
    const client = new SupervisorDaemonClient({
      socketPath: env.socketPath,
      daemonScriptPath,
    ...
    assert.equal(status.implementationVersion, "2.0.46");
