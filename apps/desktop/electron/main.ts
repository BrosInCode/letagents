...
async function fetchRoomSnapshot(requestedRoomIdentifier?: string | null): Promise<DesktopRoomSnapshot> {
  const roomIdentifier = requestedRoomIdentifier?.trim() || await resolveRoomIdentifier(workspaceRoot);
  if (!roomIdentifier) {
    return {
      roomIdentifier: null,
...
async function runDesktopRoomTaskWorkerAction(
  ...
  const session = getCurrentLocalWorkerSession(readLetAgentsLocalState(letagentsLocalStatePath), trimmedRoomIdentifier);
...
function mapRoomStreamTaskPayload(task: {
  ...
  created_by?: string | null;
  pr_url?: string | null;
  workflow_artifacts?: Parameters<typeof mapDesktopTaskSummaryPayload>[0]["workflow_artifacts"];
  ...
  stale_prompt_state?: Parameters<typeof mapDesktopTaskSummaryPayload>[0]["stale_prompt_state"];
  created_at?: string | null;
  updated_at?: string;
  updatedAt?: string;
...
async function startDeviceAuthFlow(roomIdentifier?: string | null): Promise<DesktopAuthStartResult> {
  const trimmedRoomIdentifier = roomIdentifier?.trim() || await resolveRoomIdentifier(workspaceRoot);
...
function buildWorkerSnapshots(): WorkerSnapshot[] {
  return buildLocalWorkerSnapshots(readLetAgentsLocalState(letagentsLocalStatePath));
}
...
ipcMain.handle("desktop:repos:get-status", async (): Promise<RepoStatus> => buildRepoStatus(workspaceRoot));
