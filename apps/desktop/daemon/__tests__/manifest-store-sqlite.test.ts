...
test("future SQLite schema versions are rejected without being downgraded", async () => {
  ...
  const store = new ManifestStore(env.databasePath);
  try {
    await assert.rejects(() => store.load(), /schema version 6/);
...
test("manifest metadata schema disagreement is rejected on reopen", async () => {
  ...
  try {
    ...
    const database = new DatabaseSync(env.databasePath);
    database.prepare("UPDATE manifest_metadata SET schema_version = 6 WHERE singleton = 1").run();
    database.close();
    const reopened = new ManifestStore(env.databasePath);
    await assert.rejects(() => reopened.load(), /metadata schema version 6/);
    await reopened.close();
  } finally {
    await initialized.close();
    await env.cleanup();
...
test("contradictory SQLite and metadata version pairs reject before migration", async () => {
  for (const pair of [
    ...
    { userVersion: 1, metadataVersion: 6, pattern: /metadata schema version 6/ },
  ]) {
    const env = await fixture();
    const initialized = new ManifestStore(env.databasePath);
    try {
...
test("v1 daemon state migrates transactionally to v2 and normalizes exit timestamps", async () => {
  ...
  try {
    ...
    assert.equal((inspection.prepare("SELECT schema_version FROM manifest_metadata WHERE singleton = 1").get() as { schema_version: number }).schema_version, 5);
...
test("partially migrated v2 state is repaired transactionally before reads", async () => {
  ...
  try {
    ...
    partial.prepare("DELETE FROM reconciliation_exit_timestamps WHERE agent_id = ?").run(entry.id);
    assert.equal((partial.prepare("PRAGMA user_version").get() as { user_version: number }).user_version, 5);
    assert.equal((partial.prepare("SELECT schema_version FROM manifest_metadata WHERE singleton = 1").get() as { schema_version: number }).schema_version, 5);
    partial.close();
