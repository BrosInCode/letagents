 ...
 *  - "target": authority over one workspace path while provisioning it.
 *  - "repo": authority over the whole namespace for live generations and GC.
 */
...
async function readRecord(path: string): Promise<FenceRecord | null> {
  try {
    ...
    // Records written before scope existed were namespace-wide by construction.
    // Reading a missing or unknown scope as repo keeps mixed-version upgrades safe.
...
/** Returns whether an incoming fence conflicts with one live record. */
function fenceConflicts(incoming: FenceRecord, existing: FenceRecord): boolean {
  // Repository-wide GC excludes every live record, in both acquisition orders.
  if (incoming.scope === "repo" && incoming.mode === "exclusive") return true;
  if (existing.scope === "repo" && existing.mode === "exclusive") return true;
  // Provisioning is exclusive only for the exact target workspace. A retained
  // live generation on that same target must still block reprovisioning, while
  // sibling paths remain independent. Apply the rule symmetrically so a
  // generation cannot start in the middle of a target-local mutation either.
  if ((incoming.scope === "target" && incoming.mode === "exclusive")
    || (existing.scope === "target" && existing.mode === "exclusive")) {
 ...
 * generation. Provisioning takes a target-exclusive fence, so independent
 * workspaces can start concurrently. GC takes repo-exclusive authority and
 * remains blocked by every live record until explicit quiescence. The mutation
 * lock keeps transitions atomic; PID-stamped records make crash recovery safe.
 */
...
/** Compatibility helper for a short, target-local exclusive provision. */
