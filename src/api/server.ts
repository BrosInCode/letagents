...
app.post("/projects", async (req: AuthenticatedRequest, res) => {
  ...
  res.status(201).json({ id: project.id, code: project.code ?? project.id, name: project.name ?? project.id });
});
...
app.get("/projects/join/:code", async (req, res) => {
  ...
  res.json({ id: project.id, code: project.code ?? project.id, name: project.name ?? project.id });
});
...
app.post("/projects/room/:name", async (req: AuthenticatedRequest, res) => {
  ...
  res.status(created ? 201 : 200).json({ id: project.id, code: project.code ?? project.id, name: project.name ?? project.id });
});
