...
function parseAdmission(body: Record<string, unknown>, revision: boolean) {
  const supervisorGrantId = requiredString(body, "supervisor_grant_id");
  const roomId = requiredString(body, "room_id");
  const agentKey = requiredString(body, "agent_key");
  const approverAccountId = requiredString(body, "approver_account_id");
  const expiresAt = requiredString(body, "expires_at");
  const clientRequestId = requiredString(body, "client_request_id");
